...
struct VideoCallView: View {
    ...
    init(target: CallTarget, minimized: Bool, onToggleMinimize: @escaping () -> Void, onEnd: @escaping () -> Void, pipManager: CallPiPManager) {
        self.target = target
    ...
    var body: some View {
        ...
        .onChange(of: vm.status) { status in
            if status == .connected, let remoteTrack = vm.remoteVideoTrack {
                setupPiPIfNeeded(remoteTrack: remoteTrack)
            }
        }
        .onChange(of: vm.remoteVideoTrack != nil) { hasTrack in
            if hasTrack, vm.status == .connected, let remoteTrack = vm.remoteVideoTrack {
                setupPiPIfNeeded(remoteTrack: remoteTrack)
            }
        }
    }

    private func setupPiPIfNeeded(remoteTrack: RTCVideoTrack) {
        guard let scene = UIApplication.shared.connectedScenes.first as? UIWindowScene,
              let window = scene.windows.first else { return }
        pipManager.setup(sourceView: window, remoteTrack: remoteTrack)
    }
